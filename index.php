...
<body>
  ...
  $result = $conn->query($sql);
    if ($result->num_rows > 0)  /* num_rows give back a number of row in db table */
	{
	  ...
	  /* average temperature of the devices we have $deviceTemperature / number of devices */
        try{
          $avargeTemperature = $avargeTemperature / $result->num_rows;
          $avargeHumidity = $avargeHumidity / $result->num_rows;
          $avargeCarbon = $avargeCarbon / $result->num_rows;
          $avargeGas = $avargeGas / $result->num_rows;
